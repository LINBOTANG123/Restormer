...
def load_mri_data(mat_path, key='k_gc'):
    ...
    This function takes only the first sample (index=0) and converts each
    ...
    # Select the first sample along the last axis.
...
def load_noise_data(mat_path, key='k_gc'):
    ...
    Takes the first sample (index=0) and processes each 2D slice with IFFT 
...
def main():
    ...
    # Load MRI data; expected shape after processing: (H, W, num_coils, mri_slices) = (146,146,32,40)
    mri_img = load_mri_data(args.mri_mat, key='Kimage_first_full')
    H, W, num_coils, mri_slices = mri_img.shape
    print("MRI image shape (first sample):", mri_img.shape)
    ...
    # Load noise data; expected shape after processing: (H, W, num_coils, noise_slices) = (146,146,32,40)
