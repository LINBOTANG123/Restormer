...
# Number of coils (assumed to be 32)
NUM_COILS = 32
...
def load_model(model_path, device='cuda'):
    """
    Load the multi-coil Restormer model configured for the (1+1+31) input format.
    inp_channels=33 (target coil noisy image + its noise map + 31 other noisy coils)
...
def main():
    parser = argparse.ArgumentParser(description="Inference for multi-coil MRI denoising with (1+1+31) input and (1) output.")
    ...
    parser.add_argument('--data_scale_factor', type=float, default=1e5, help='Scaling factor for data (applied to both MRI and noise maps)')
    args = parser.parse_args()
    os.makedirs(args.output_folder, exist_ok=True)

    # Load MRI data; expected shape: (H, W, num_coils, mri_slices)
    ...
    # Rearranged to (H, W, mri_slices, num_coils)
    ...
    # Load model configured for 33-channel input and 1-channel output.
    model = load_model(args.model_pth, device='cuda')

    # Prepare lists to store outputs for all slices.
    # For each slice, we will produce outputs for all 32 coils.
    all_denoised = []  # list of shape: (mri_slices, num_coils, H, W)
    all_residual = []  # list of shape: (mri_slices, num_coils, H, W)

    # Process each slice.
    for s in range(mri_slices):
        # Extract coil images and noise maps for slice s.
        # coil_imgs: (H, W, num_coils) and noise_slice: (H, W, num_coils)
        coil_imgs = mri_img_scaled[:, :, :, s]
        noise_slice = noise_maps_scaled[:, :, :, s]
        # Transpose to (num_coils, H, W)
        coil_imgs = np.transpose(coil_imgs, (2, 0, 1))
        noise_slice = np.transpose(noise_slice, (2, 0, 1))
        slice_denoised = []  # list to store denoised output for each coil in this slice
        slice_residual = []  # list to store residual for each coil in this slice
        for target_idx in range(num_coils):
            # Build input with 33 channels:
            # 1. Target coil noisy image.
            target_noisy = coil_imgs[target_idx:target_idx+1, :, :]
            # 2. Target coil noise map.
            target_noise_map = noise_slice[target_idx:target_idx+1, :, :]
            # 3. The remaining 31 noisy coil images.
            rest_list = [coil_imgs[i:i+1, :, :] for i in range(num_coils) if i != target_idx]
            rest_noisy = np.concatenate(rest_list, axis=0)
            # Concatenate to form input of shape (33, H, W).
            input_array = np.concatenate([target_noisy, target_noise_map, rest_noisy], axis=0)
            input_tensor = torch.from_numpy(input_array).unsqueeze(0).float().cuda()
            ...
            # Compute residual: difference between target noisy image (first channel) and prediction.
            target_noisy_tensor = input_tensor[:, 0:1, :H_in, :W_in]
            residual = target_noisy_tensor - pred

            slice_denoised.append(pred.cpu())
            slice_residual.append(residual.cpu())
            print(f"Processed slice {s+1}/{mri_slices}, coil {target_idx+1}/{num_coils}")
        # Stack outputs for this slice: shape -> (num_coils, 1, H, W); then remove singleton channel.
        slice_denoised = torch.cat(slice_denoised, dim=0)[:, 0, :, :]  # shape: (num_coils, H, W)
        slice_residual = torch.cat(slice_residual, dim=0)[:, 0, :, :]  # shape: (num_coils, H, W)
        all_denoised.append(slice_denoised)
        all_residual.append(slice_residual)
    
    # Stack along slice dimension.
    # Now we have: (mri_slices, num_coils, H, W)
    denoised_vol = torch.stack(all_denoised, dim=0).numpy()
    residual_vol = torch.stack(all_residual, dim=0).numpy()
    ...
    # Save denoised and residual volumes as NIfTI files.
