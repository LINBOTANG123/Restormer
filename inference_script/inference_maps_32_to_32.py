...
def load_model(model_path, device='cuda'):
    ...
    # Model configured for 64-channel input (32 noisy coils + 32 noise maps) and 32-channel output (one denoised per coil)
    net_g = Restormer(
        inp_channels=32,
        out_channels=32,
        dim=48,
...
def run_inference_from_mat(model_pth, mri_mat_file, noise_mat_file, output_folder, device='cuda',
    ...
    For each slice and for each coil, a 2-channel input is formed by stacking the scaled
    MRI image (one channel) and the corresponding scaled noise map (one channel).
    The model (configured with inp_channels=64 and out_channels=32) processes the combined
    multi-coil input, but here we extract per-coil inputs by grouping 32 coils.
    This script then saves per-slice, per-coil PNG outputs for:
      - original MRI images (scaled)
      - denoised outputs (per coil)
      - residuals (original minus denoised)
    Finally, the outputs for all slices are assembled into 4D NIfTI volumes with dimensions
    (H, W, num_slices, num_coils).
    """
    ...
    original_slices = []   # List of arrays (num_coils, H, W) per slice.
    denoised_slices = []   # List of arrays (num_coils, H, W) per slice.
    residual_slices = []   # List of arrays (num_coils, H, W) per slice.
    ...
    for slice_idx in range(num_slices):
        ...
        # For multi-coil input, concatenate all 32 MRI images and 32 noise maps.
        # Input shape: (2*num_coils, H, W)
        # input_multi_coil = np.concatenate([mri_slice, noise_maps], axis=0)
        input_multi_coil = mri_slice
        ...
        # Output shape is expected to be (1, 32, H, W)
        output = output[0].detach().cpu().numpy()
        denoised_slices.append(output)
        
        # Compute residual per coil.
        residual = mri_slice - output  # Both are scaled images in [0,1] range.
    ...
    original_vol = np.transpose(original_vol, (2, 3, 0, 1))
...
def parse_args():
    parser = argparse.ArgumentParser(description="Inference from MAT inputs (64-ch input, 32-ch output)")
    ...
    parser.add_argument('--data_scale_factor', type=float, default=2e5,
