...
# Number of coil groups remains 32, but output is now a single combined channel.
NUM_COILS = 32
...
def load_model(model_path, device='cuda'):
    ...
    The model is now configured with:
      ...
      - out_channels=1 (a single combined denoised image)
...
def main():
    parser = argparse.ArgumentParser(description="Inference for multi-coil MRI denoising with 1-channel output.")
    ...
    # Apply data scaling.
    # (If you prefer normalization instead of scaling, replace this section with normalization.)
    ...
    input_slices = []
    for s in range(mri_slices):
        coil_imgs = mri_img_scaled[:, :, :, s]      # shape: (H, W, num_coils)
        noise_slice = noise_maps_scaled[:, :, :, s]   # shape: (H, W, num_coils)
    ...
    for s in range(mri_slices):
        ...
        # Compute the average of the original coil images (first 32 channels) to compare
        original_coil_avg = torch.mean(slice_input[:, :32, :H_in, :W_in], dim=1, keepdim=True)
        residual = pred - original_coil_avg
