...
def interleave_channels(x):
    ...
    The model's forward function will call this.
    """
    B, C, H, W = x.shape
...
def normalize_percentile(data, lower_percentile=5, upper_percentile=95):
    """
    Normalizes the data to the [0, 1] range using the specified lower and upper percentiles.
    Values below the lower percentile and above the upper percentile are clipped.
    """
    lower = np.percentile(data, lower_percentile)
    upper = np.percentile(data, upper_percentile)
    # Avoid division by zero in case upper == lower
    if upper - lower == 0:
...
def save_png(image, path):
    """
    Saves a 2D image (assumed to be in [0,1]) as an 8-bit PNG.
    """
...
def main():
    ...
    # Create subfolders for PNGs.
    normalized_input_folder = os.path.join(args.output_folder, "normalized_input_pngs")
    denoised_output_folder = os.path.join(args.output_folder, "denoised_output_pngs")
    os.makedirs(normalized_input_folder, exist_ok=True)
    ...
    # Save the original MRI data (before normalization) as a 4D NIfTI volume.
    ...
    # Normalize the data using the lower 5th and upper 95th percentiles.
    mri_img_norm = normalize_percentile(mri_img, lower_percentile=5, upper_percentile=95)
    noise_maps_norm = normalize_percentile(noise_maps, lower_percentile=5, upper_percentile=95)
    
    # Save the normalized original MRI data as a 4D NIfTI volume.
    # Rearrange from (H, W, num_coils, mri_slices) to (H, W, mri_slices, num_coils)
    normalized_vol = np.transpose(mri_img_norm, (0, 1, 3, 2))
    normalized_nii_path = os.path.join(args.output_folder, "original_4d_normalized.nii")
    nib.save(nib.Nifti1Image(normalized_vol.astype(np.float32), affine=np.eye(4)), normalized_nii_path)
    print(f"Saved normalized original volume as {normalized_nii_path}")
    ...
    for s in range(mri_slices):
        coil_imgs = mri_img_norm[:, :, :, s]      # (H, W, num_coils)
        noise_slice = noise_maps_norm[:, :, :, s]   # (H, W, num_coils)
    ...
    for s in range(mri_slices):
        ...
        # Save normalized input PNGs for each coil.
        # mri_img_norm is in shape (H, W, num_coils, mri_slices).
        for coil in range(num_coils):
            coil_img = mri_img_norm[:, :, coil, s]  # shape: (H, W)
            input_png_path = os.path.join(normalized_input_folder, f"slice_{s:03d}_coil_{coil:02d}.png")
            save_png(coil_img, input_png_path)
        
        # Save denoised output PNGs for each coil.
        # pred is of shape [1, 32, H, W]. We remove the batch dimension.
        pred_np = pred.squeeze(0).cpu().numpy()  # shape: (32, H, W)
        for coil in range(NUM_COILS):
            coil_pred = pred_np[coil, :, :]  # shape: (H, W)
            denoised_png_path = os.path.join(denoised_output_folder, f"slice_{s:03d}_coil_{coil:02d}.png")
            save_png(coil_pred, denoised_png_path)
    ...
    # Since the model was trained on natural images normalized to [0, 1],
    # the outputs are in the [0, 1] range. No re-scaling is necessary.
