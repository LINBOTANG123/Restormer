...
class Dataset_OnlineGaussianDenoising(torch.utils.data.Dataset):
    """
    An on-the-fly denoising dataset that simulates multi-coil MRI data using a CSM.
    ...
    For each GT image sample, the pipeline is:
      ...
      5) Generate clean coil images using the CSM:
             clean_coil[i] = GT image * |CSM[i]|
      6) For each coil, add a small whole-image noise, compute a coil-specific smoothed image,
         and add spatially varying noise.
      7) Concatenate the noisy coil images with the corresponding noise maps to form an input
         with 60 channels (32 noisy coils + 32 noise maps).
      8) Combine the 32 clean coil images using RSS to form a single-channel GT.
    """
    def __init__(self, opt):
    ...
    def __getitem__(self, index):
        ...
        offset_range = self.opt.get('intensity_offset_range', 0.4)  # can be adjusted as needed
        ...
        # --- Generate clean coil images using the CSM ---
        ...
        # --- Create GT RSS image (single channel) ---
        gt_rss = np.sqrt(np.sum(np.square(coil_clean), axis=0))
        if gt_rss.max() > 0:
            gt_rss = gt_rss / gt_rss.max()
        ...
        for i in range(self.num_coils):
            ...
            coil_noisy_with_coil_noise = coil_clean_i + smoothing_noise
            # Now, add whole-image noise after adding the coil noise
            whole_noise = np.random.randn(H, W).astype(np.float32) * self.whole_noise_std
            coil_noisy[i] = coil_noisy_with_coil_noise + whole_noise
            coil_noisy[i] = np.clip(coil_noisy[i], 0, 1)


        # --- Form Input: Concatenate 32 noisy coil images and 32 noise maps (total 64 channels) ---
        lq = np.concatenate([coil_noisy, noise_map_array], axis=0)  # shape: (64, H, W)
        # GT for training is the RSS combined clean image.
        gt_final = gt_rss  # shape: (H, W)

        # --- Convert to torch tensors ---
        lq_tensor = torch.from_numpy(lq).float()
        gt_tensor = torch.from_numpy(gt_final).float().unsqueeze(0)  # add channel dim -> (1, H, W)

        return {
            'lq': lq_tensor,  # Input: 64 channels (32 noisy coils + 32 noise maps)
            'gt': gt_tensor,  # Target: 1 channel (RSS combined clean image)
            'lq_path': gt_path,
            'gt_path': gt_path,
