...
class Dataset_OnlineGaussianDenoising(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        opt dict must contain:
          ...
          'use_csm' (bool): if True, use the coil sensitivity map from a MAT file;
                            if False, replicate the GT image for multi-coil simulation.
        ...
        # Whole-image noise (constant noise added to the entire image)
    ...
    def __getitem__(self, index):
        ...
        # 2. Crop the GT image to the desired size
        ...
        # 5. Squeeze GT image to (H, W)
        img_gt = np.squeeze(img_gt, axis=2)  # shape: (H, W)
        H, W = img_gt.shape

        # 6. Generate clean coil images:
        if self.use_csm:
            # Use the sensitivity map:
            # Randomly select one version (slice) from the sensitivity map.
            ...
            # Simply replicate the GT image for each coil.
            coil_clean = np.repeat(img_gt[np.newaxis, ...], self.num_coils, axis=0)

        # 7. For each coil, add noise:
        coil_noisy = np.zeros_like(coil_clean)
        noise_map_array = np.zeros_like(coil_clean)
        noise_std_list = []  # store the noise std used for each coil

        do_invert = (random.random() < self.random_invert_prob)

        for i in range(self.num_coils):
            # (a) Add a small whole-image noise (a constant noise value for this coil)
            whole_noise = np.random.randn() * self.whole_noise_std
            coil_clean_with_whole_noise = coil_clean[i] + whole_noise

            # (b) Compute a coil-specific smoothed version using the noisy image.
            ...
            # (c) Sample an independent noise std for this coil (for spatially varying noise)
            ...
            # Generate spatially varying noise for this coil.
            smoothing_noise = np.random.randn(H, W).astype(np.float32) * noise_map_coil
            # Final noisy coil image = coil image with whole noise + spatially varying noise.
        ...
        #    This results in an input with (num_coils + num_coils) channels.
