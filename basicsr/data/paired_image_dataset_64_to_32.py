...
class Dataset_OnlineGaussianDenoising(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.whole_noise_std = opt.get('whole_noise_std', 0.02)
        ...
        self.use_csm = opt.get('use_csm', True)
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.num_coils):
            # Use the original clean coil image without whole noise
            coil_clean_i = coil_clean[i]  # shape: (H, W)

            # Apply repeated Gaussian smoothing to the clean coil image
            ...
            # Generate coil-specific noise standard deviation and compute the noise map
            ...
            # Generate coil-specific (smoothing) noise based on the noise map
            smoothing_noise = np.random.randn(H, W).astype(np.float32) * noise_map_coil

            # Add the coil-specific noise to the original clean image
