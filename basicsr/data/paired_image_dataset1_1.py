...
class Dataset_OnlineGaussianDenoising(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        target_coil = 0  # fixed target coil index (can be randomized if desired)
        ...
        # Generate spatially varying noise using the smoothed image as a scale factor.
        ...
        # --- Form Input and GT ---
        # Input: single channel (noisy coil image)
        lq_tensor = torch.from_numpy(noisy_coil).float().unsqueeze(0)  # shape: (1, H, W)
        # GT: the clean coil image (target)
        gt_tensor = torch.from_numpy(clean_coil).float().unsqueeze(0)  # shape: (1, H, W)

        return {
            'lq': lq_tensor,  # Input: single channel noisy image
            'gt': gt_tensor,  # Target: single channel clean image
            'lq_path': gt_path,
            'gt_path': gt_path,
            'noise_std': noise_std,
        }
