...
class Dataset_OnlineGaussianDenoising(data.Dataset):
    """
    An on-the-fly denoising dataset that simulates multi-coil MRI data using a coil sensitivity map.
    
    For each GT image sample, the pipeline is:
      1) Crop the image to the sensitivity map’s spatial dimensions.
      2) Optionally apply data augmentation.
      3) Add a random intensity offset.
      4) Use the sensitivity map to generate clean coil images.
      5) For each coil, add whole-image noise and then apply repeated Gaussian smoothing
         to generate a coil-specific noise map. Use the noise map to add spatially varying noise.
      6) Randomly choose one coil as the target. Construct the input by concatenating:
           - The target coil’s noisy image,
           - Its corresponding noise map,
           - The noisy images of the remaining coils.
      7) The target (gt) is set as the clean (noise-free) image of the chosen coil.
    """
    ...
    def __init__(self, opt):
        ...
        opt dict must contain:
          ...
          'coil_sens_path' (str): path to the .mat file containing the coil sensitivity map.
          'intensity_offset_range' (float): range for random intensity offset.
          ... (others as needed)
        """
        ...
        # Multi-coil simulation parameters (CSM only)
        self.num_coils = opt.get('num_coils', 32)
        self.use_csm = True  # only support CSM in this version
    ...
    def __getitem__(self, index):
        ...
        # --- Load, crop, and optionally augment the GT image ---
        ...
        # *** Change intensity by adding a random offset (shift the mean) ***
        offset_range = self.opt.get('intensity_offset_range', 0.4)
        intensity_offset = random.uniform(-offset_range, offset_range)
        img_gt = np.clip(img_gt + intensity_offset, 0, 1.0)

        # Remove the singleton channel; now shape: (H, W)
        img_gt = np.squeeze(img_gt, axis=2)
        H, W = img_gt.shape

        # --- Generate clean coil images using the sensitivity map ---
        ...
        # --- For each coil, add noise to simulate a noisy acquisition ---
        coil_noisy = np.zeros_like(coil_clean)
        noise_map_array = np.zeros_like(coil_clean)
        global_noise_std_list = []  # store noise std per coil
        for i in range(self.num_coils):
            whole_noise = np.random.randn() * self.whole_noise_std
            coil_clean_with_noise = coil_clean[i] + whole_noise

            coil_smoothed = repeated_gaussian_smoothing(
                coil_clean_with_noise,
            ...
            global_noise_std_list.append(coil_noise_std)

            noise_map = coil_smoothed * coil_noise_std
            noise_map_array[i] = noise_map

            smoothing_noise = np.random.randn(H, W).astype(np.float32) * noise_map
            coil_noisy[i] = np.clip(coil_clean_with_noise + smoothing_noise, 0, 1)

        # --- Randomly choose one coil as the target ---
        target_idx = random.randint(0, self.num_coils - 1)

        # --- Build final input (lq) ---
        # lq: [target's noisy image, target's noise map, all other noisy coil images]
        target_noisy = np.expand_dims(coil_noisy[target_idx], axis=0)
        target_noise_map = np.expand_dims(noise_map_array[target_idx], axis=0)
        # Select the rest (preserving order, excluding target_idx)
        rest_noisy = np.concatenate([np.expand_dims(coil_noisy[i], axis=0)
                                     for i in range(self.num_coils) if i != target_idx], axis=0)
        lq = np.concatenate([target_noisy, target_noise_map, rest_noisy], axis=0)
        # Final lq shape: (2 + (num_coils - 1), H, W) = (num_coils + 1, H, W)

        # --- Set ground truth as the clean version of the target coil ---
        gt = coil_clean[target_idx]  # shape: (H, W)
        ...
        return {
            'lq': lq_tensor,         # Input: target noisy image, its noise map, and remaining noisy coils
            'gt': gt_tensor,         # Target: clean image of the chosen coil
            'lq_path': gt_path,
            'gt_path': gt_path,
            'global_noise_std': global_noise_std_list[target_idx],
        }
